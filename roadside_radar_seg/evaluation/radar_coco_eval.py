...
class RadarCOCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType="pointcloud"):
        """
        ...
        """
        if not iouType:
            print("iouType not specified. use default iouType pointcloud")
    ...
    def _prepare(self):
        ...
        self._dts = defaultdict(list)  # dt for evaluation
        for gt in gts:
            self._gts[gt["pcd_id"], gt["category_id"]].append(gt)
        for dt in dts:
            self._dts[dt["pcd_id"], dt["category_id"]].append(dt)
    ...
    def evaluate(self):
        ...
        """
        tic = time.time()
        print("Running per pcd evaluation...")
        p = self.params
        ...
        elif p.iouType == "pointcloud":
            computeIoU = self.computePc2Iou
        self.ious = {
    ...
    def computePc2Iou(self, imgId, catId):  # for one image id and one class
        p = self.params
        if p.useCats:
        ...
        if p.iouType == "segm":
            ...
            g = [g["clusters"] for g in gt]  # list of lists
            d = [d["clusters"] for d in dt]
        else:
        ...
        # ious = np.zeros((m * n,), dtype=np.double)

        ious = np.zeros((m, n), dtype=np.double)

        for gt_idx in range(n):
            gt_cluster = Multiset(
                [tuple(x) for x in torch.round(g[gt_idx], decimals=2).cpu().numpy()]
            )
            for pred_idx in range(m):
                pred_cluster = Multiset(
                    [tuple(x) for x in torch.round(d[pred_idx], decimals=2).cpu().numpy()]
                )
                intersection = len(gt_cluster & pred_cluster)
                union = len(gt_cluster | pred_cluster)

                ious[pred_idx, gt_idx] = intersection / union
                # ious[pred_idx, gt_idx] = iou (gt_cluster, pred_cluster)
        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    gtm[tind, m] = d["id"]
        # set unmatched detections outside of area range to ignore TODO
    ...
    def summarize(self):
        ...
        elif iouType == "pointcloud":
            summarize = _summarizePc
...
class Params:
    ...
    def __init__(self, iouType="pointcloud"):
        if iouType == "pointcloud":
            self.setDetParams()
        # elif iouType == "keypoints":
        #     self.setKpParams()
        else:
